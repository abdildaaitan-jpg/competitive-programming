#include <bits/stdc++.h>
 
using ll = long long;
#define sz(v) ((int)v.size())
#define se second
#define fi first
#define upb upper_bound
#define lwb lower_bound
#define pb push_back
#define eb emplace_back
#define int ll
#define pofik continue
#define all(v) v.begin(), v.end()
#define rall(v) v.rbegin(), v.rend()
// #define get(v,n) get<n>(v)
using namespace std;
const ll maxn = 1e18 + 7;
const ll MOD = 1e9 + 7;
const ll N = 10;
int gcd(int a, int b) {if (b == 0) return a; return gcd(b, a % b);}
int lcm(int a, int b) {return a / gcd(a, b) * b;}
int binpow(int a, int b) {
    int res = 1; 
    while (b > 0) { if (b & 1) res = (res * a) % MOD; a = (a * a) % MOD; b /= 2; }
    return res;
}
void up(int &x, int y) { x = max(x, y); return; }
void down(int &x, int y) { x = min(x, y); return; }
 
int n, m;
vector < string > g;
vector < vector < bool >> used;
bool cycle = 0;
vector < int > du = {0, 0, 1, -1}, dv = {1, -1, 0, 0}; 

void dfs(int u, int v, int pu, int pv) {
    if (cycle) return;

    used[u][v] = 1;

    for (int i = 0; i < 4; i++) {
        int nu = u + du[i], nv = v + dv[i];
        
        if (nu < 0 || nu >= n || nv < 0 || nv >= m) pofik;
        if (nu == pu && nv == pv) pofik;

        if (used[nu][nv] && g[u][v] == g[nu][nv]) {
            cycle = 1;
            return;
        }
        else if (g[u][v] == g[nu][nv]) {
            dfs(nu, nv, u, v);
        }
    }
    return;
}

void solve() {
    cin >> n >> m;
    g.resize(n); used.resize(n, vector < bool > (m, 0));
    for (auto &it: g) cin >> it;

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (!used[i][j]) {
                dfs(i, j, -1, -1);
            }
            if (cycle) {
                cout << "Yes";
                return;
            }
        }
    }
    if (cycle) cout << "Yes";
    else cout << "No";
    return;
}
 
signed main() {
    // freopen("cowdance.in", "r", stdin);
    // freopen("cowdance.out", "w", stdout);
    ios::sync_with_stdio(false); cin.tie(nullptr);
    int T = 1;
    // cin >> T;
    while (T--) solve();
    return 0;
}